...
public class SlideLayout extends FrameLayout {
    ...
    private void performAnim(float percent) {
        ...
        // 左侧菜单从0.8的大小，放大到1.0
        mMenuContainer.setScaleX(evaluate(percent, 0.8f, 1.0f));
        mMenuContainer.setScaleY(evaluate(percent, 0.8f, 1.0f));

        // 主容器从1.0缩小到0.7
        mMainContainer.setScaleX(evaluate(percent, 1.0f, 0.7f));
        mMainContainer.setScaleY(evaluate(percent, 1.0f, 0.7f));

        // 菜单平移
        mMenuContainer.setTranslationX(evaluate(percent, -mSlideWidth / 2.0f, 0));

        // 菜单渐变
        mMenuContainer.setAlpha(evaluate(percent, 0.2f, 1.0f));
